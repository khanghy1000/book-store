...
class KnnModel:
    ...
    def train_model(self, model_path):
        ...
        self.data["cleaned_text"] = self.data["cleaned_text"].apply(self.preprocess_text)

        clean_description = self.cv.fit_transform(self.data['cleaned_text']).toarray()
        self.model.fit(clean_description)
    ...
    def recommend(self, text, k=5):
        cleaned_text = self.cv.transform([self.preprocess_text(text)]).toarray()
